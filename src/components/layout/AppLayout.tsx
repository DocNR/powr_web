    ...
    >
      <div className="min-h-screen bg-[var(--color-surface-base)] flex">
        ...
        <div className={`flex-1 flex flex-col overflow-x-hidden max-w-full ${!isMobile ? 'ml-64' : ''}`}>
          ...
          <div className={`${isMobile ? 'fixed top-0 left-0 right-0 z-40' : 'sticky top-0 z-40'} bg-[var(--color-surface-card)]`}>
            <AppHeaderWithWorkoutContext />
          </div>

          {/* Conditional Sub-Navigation - Fixed header for tabs that need it */}
          {isMobile && subNavItems && (
            <div className="fixed top-12 left-0 right-0 z-30 bg-[var(--color-surface-card)]">
              <SubNavigation
                items={subNavItems}
